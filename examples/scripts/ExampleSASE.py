...
# To fix the particle orientation, set a random seed in get_random_quat in skopi/geometry/generate.py
...
# Case2: SASE Beam, SPI
Beam = sk.Beam
sase = sk.SASEBeam(mu=7120, sigma=10, n_spikes=100, fname=beamfile)
increase_factor = 1e2
...
tic = time.time()
experiment = sk.SPIExperiment(det, sase, particle)
img = experiment.generate_image()
toc = time.time()
