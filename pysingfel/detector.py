...
########################################################################
    ...
    ## Notice that the background noise enter photons and ADUs in different ways.
    ## In photons, background noise enter as a photons while in ADU it enter as noise.
    def photons(self, particle, beam, device = "cpu"):
        raw_data = self.get_pattern_without_corrections(particle, beam, device)
        return (self.add_correction_and_quantization(scaling_tmp(raw_data))+
                np.random.poisson(np.random.uniform( 0, 2* np.sqrt(3*self.pixel_rms))/130 ))
    
    def photons_without_static_noise(self, particle, beam, device = "cpu"):
        raw_data = self.get_pattern_without_corrections(particle, beam, device)
        return self.add_correction_and_quantization(scaling_tmp(raw_data))
    
    def ADU(self, particle, beam, path, device = "cpu"):
...
def scaling_tmp(pattern):
    normalization_factor = 839699./np.sum(pattern)
