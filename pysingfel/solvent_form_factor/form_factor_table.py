...
class FormFactorTable(object):
    _CARBON_ATOMS_DICT = {
        ...
        'CB': defaultdict(lambda: 'CH2',
            {'ILE': 'CH', 'THR': 'CH', 'VAL': 'CH', 'ALA': 'CH3'}),
        'CG': defaultdict(lambda: 'CH2',
            {'ASN': 'C', 'ASP': 'C', 'HIS': 'C', 'PHE': 'C',
             'TRP': 'C', 'TYR': 'C', 'LEU': 'CH'}),
        'CG1': {'ILE': 'CH2', 'VAL': 'CH3'},  # No default
        'CG2': defaultdict(lambda: 'CH3'),
        'CD': defaultdict(lambda: 'CH2', {'GLU': 'C', 'GLN': 'C'}),
        'CD1': defaultdict(lambda: 'C',
            {'LEU': 'CH3', 'ILE': 'CH3', 'PHE': 'CH',
            'TRP': 'CH', 'TYR': 'CH'}),
        'CD2': defaultdict(lambda: 'C',
            {'LEU': 'CH3', 'PHE': 'CH', 'HIS': 'CH', 'TYR': 'CH'}),
        'CE': defaultdict(lambda: 'C', {'LYS': 'CH2', 'MET': 'CH3'}),
        'CE1': defaultdict(lambda: 'C',
            {'PHE': 'CH', 'HIS': 'CH', 'TYR': 'CH'}),
        ...
        'C5': defaultdict(lambda: 'C',
            {'DCYT': 'CH', 'CYT': 'CH', 'DURA': 'CH', 'URA': 'CH'}),
        'C6': defaultdict(lambda: 'C',
            {'DCYT': 'CH', 'CYT': 'CH', 'DURA': 'CH', 'URA': 'CH',
            'DTHY': 'CH', 'THY': 'CH'}),
    ...
    def __init__(self, table_name=None, mnq=0.0, mxq=3.0, dq=0.01):
        ...
        self.ff_cm_dict = {
            'H':0, 'HE':1, 'C':2, 'N':3, 'O':4, 'NE':5, 'SOD+':6, 'MG+2':7,
            'P':8, 'S':9, 'CAL2+':10, 'FE2+':11, 'ZN2+':12, 'SE':13, 'AU':14,
            'CH':15, 'CH2':16, 'CH3':17, 'NH':18, 'NH2':19, 'NH3':20,
            'OH':21, 'OH2':22, 'SH':23}
        ...
        self.ff_coeff = np.array([
            [0.493002, 0.322912, 0.140191, 0.040810, 0.0, 0.003038, 10.5109, 26.1257, 3.14236, 57.7997, 1.0, 5.15],
        ...
        self.vacuum_form_factors = np.zeros(
            (self.ff_coeff.shape[0], self.number_of_q_entries),
            dtype=np.float64)
        self.dummy_form_factors = np.zeros(
            (self.ff_coeff.shape[0], self.number_of_q_entries),
            dtype=np.float64)
    ...
    def compute_dummy_form_factors(self):
        ...
        for i in range(self.ff_coeff.shape[0]):
            for iq in range(self.number_of_q_entries):
                dff[i, iq] = (
                    self.rho * self.excl_vol[i]
                    * np.exp(-np.power(self.excl_vol[i],(2.0/3.0))
                             * (self.q_[iq]**2)/(16.0*np.pi)))
    ...
    def compute_form_factors_all_atoms(self):
        ...
        for i in range(0, self.ff_coeff.shape[0]):
            for iq in range(self.number_of_q_entries):
                ...
                for j in range(5):
                    self.vacuum_form_factors[i, iq] += (
                        self.a[i, j] * np.exp(-self.b[i, j]*s[iq]*s[iq]))
